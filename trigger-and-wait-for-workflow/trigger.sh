...
#
# Timeout budget must accommodate retry_run's full retry schedule:
...
timeout 240 bash -c '
  source "$GITHUB_ACTION_PATH/../shared/retry.sh"
  retry_run gh workflow run "$WORKFLOW" \
    --repo "$REPOSITORY" \
    --ref "$REF" \
    '"$input_flags"'
'
...
set -e
if [ "$code" -eq 124 ]; then
  echo "::error::Dispatch of $WORKFLOW exceeded 240s — gh workflow run exhausted retry attempts (HTTP 5xx) or hung on network I/O. The run may still have been created server-side; check $REPOSITORY actions for a recent run."
  exit "$code"
fi
if [ "$code" -ne 0 ]; then
