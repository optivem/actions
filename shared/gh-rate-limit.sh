...
#
# Source this file alongside retry.sh when a step makes many `gh` calls
...
#     wait_for_rate_limit
#     retry_run gh api "repos/$repo/releases"
#   done
#
# Why this is separate from retry.sh:
#   - retry.sh retries transient 5xx/network errors but hard-fails on
#     rate-limit 403 (retrying would burn quota faster).
#   - This helper is the caller-side complement that prevents hitting the 403
#     in the first place.
#
# Why `gh api rate_limit` bypasses retry_run: retrying a rate-limit probe on
