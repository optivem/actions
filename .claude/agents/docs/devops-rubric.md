...
- **Don't add retry without measured failure data.** Before wrapping a step in retry logic (a new composite action, `nick-fields/retry@v4`, or an inline shell loop), verify the underlying operation is **measurably flaky** — citing at least 2–3 failures in ~30 days across observed CI runs, not a single transient blip. A one-time GHCR/registry/CDN timeout is what `gh run rerun --failed` is for; building a retry wrapper for a hypothetical failure mode is **negative-ROI**. It adds maintenance surface (a new action to own forever, or a multi-file sweep), masks future incidents (a real outage that exceeds the retry budget still fails — just slower and with the diagnostic signal swallowed), and trains authors to wrap-everything-in-retry rather than measure first. **Industry signal:** mainstream actions (`docker/login-action`, `actions/checkout`, `docker/build-push-action`) do NOT add retry around their own primary operation — Docker, GitHub, and the action authors deliberately leave job-level rerun (`gh run rerun --failed`) as the correct layer for transient failures, because it preserves the failure-mode signal and avoids retry amplification across composed pipelines. **Three-part test before adding retry** — flag the change if any of these is missing: **(a)** cite a specific incident or ≥3 occurrences within 30 days, **(b)** explain why `gh run rerun --failed` (or another job-level rerun) is insufficient — typically because the same job must succeed mid-run because downstream steps within the job depend on it without a clean rerun boundary, or because the failure rate is high enough that mid-job retry meaningfully reduces aggregate MTTR; **(c)** confirm the wrapped operation is idempotent (or wrap it with idempotence — see the bounded-retry and idempotence rules below). When you do add retry, the next bullet ("bounded retry with backoff") governs *how*. Source: Google SRE book, ch. 22 "Addressing Cascading Failures" (retries amplify failures); the Mainstream-first principle at top of file.

- **Bounded retry with backoff.** Flag any action whose `runs:` block retries a transient-failure-prone operation (network call, API request, polling loop) without **(a)** an explicit maximum attempt count or deadline, and **(b)** exponential or jittered backoff between attempts. Unbounded `while true; do ... done` loops and flat-interval retries burn the caller's error budget and inflate MTTR during partial outages — a single wedged action can consume an entire pipeline's retry budget. A healthy retry loop has a ceiling (e.g. `max-attempts: 5` input, or a `timeout-seconds` input) and grows the sleep between attempts (e.g. 2s → 4s → 8s → 16s → 32s, optionally with jitter). Retry only **transient** error patterns — 5xx, network timeout, TLS handshake, connection reset; do NOT retry 4xx (client error, including 401/403/404/422 and rate-limit-as-403 unless the registry is documented to use 403 for throttling). The `retry_run` wrapper (`actions/shared/retry.sh`, exposed via `optivem/actions/retry@main`) is the reference implementation: hard-fails 4xx, retries 5xx + network patterns, fixed delays 5s → 15s → 45s. The `check-ghcr-image-tag-exists` probe is the reference for HTTP HEAD existence checks: stops on definitive codes (200/404/401/403), retries only on indeterminate codes. **DORA linkage — MTTR.** Unbounded retries are an MTTR amplifier: a 30-second transient registry outage escalates into a 30-minute pipeline wedge under flat-interval retry, so the MTTR observed by the team is dominated by the retry policy rather than the underlying fault. Source: Google SRE book, ch. 22 "Addressing Cascading Failures" (retry amplification) and ch. 3 "Embracing Risk" (error-budget framing); *Accelerate* (Forsgren, Humble, Kim) on MTTR as a DORA metric.

- **Determinism over retry — mirror external deps rather than wrap them in retry.** When a step retries an external dependency you cannot control (public registry, CDN, third-party API), prefer **eliminating the dependency variance** over wrapping it in retry logic. Farley's CD framing (Humble & Farley, *Continuous Delivery* ch. 4–5; Farley, *Modern Software Engineering*; "Stop using flaky tests" series on the Continuous Delivery YouTube channel): the deployment pipeline exists to give you a *deterministic* releasable-or-not signal. Non-determinism — including "transient" external flakes — destroys that signal because every red becomes ambiguous (real bug or flake?), and once the team learns to mash "rerun" on red builds, the trust the pipeline was built to provide is gone. Retry is a sticking plaster that hides the underlying dependency on a flaky third party; mirroring/caching removes the dependency. **Hierarchy of responses (apply highest tier first; do NOT stop at a lower tier when a higher one is feasible):**

  1. **Fix the root cause** in code you control — race conditions, shared mutable state, test isolation failures, non-deterministic application behavior. Almost every "flaky test" is a real defect masked by retry.
  2. **Remove the external dependency** — mirror public base images into your own GHCR with a refresh schedule; commit / cache the Gradle wrapper; cache npm/maven/gradle deps via `actions/cache` or `setup-*` built-in caching; vendor a build tool. The dep that produced the flake is no longer in the path.
  3. **Job-level rerun** (`gh run rerun --failed`) for true one-off blips with no observable pattern. Preserves the diagnostic signal and avoids retry amplification.
  4. **Operation-level retry** with bounded backoff — only when (1) and (2) are genuinely impossible (e.g. the GitHub API has no mirrorable surface) AND the flake rate is measured (per the "don't add retry without measured failure data" rule above).
  5. **Tolerate the flake rate** — the pragmatic-industry default below ~1–2%; under Farley's CD-purist position, never. The rubric's working position is: **acceptable only when (1)–(4) have been honestly considered and rejected, with reasoning written down in the action or workflow.**

  **Flag any tier-4 retry where a tier-2 mirror/cache is feasible.** Examples seen in this repo:

  - `Pre-pull base images (retry transient registry 403s)` (commit-stage workflows pulling `eclipse-temurin:*-alpine`, `mcr.microsoft.com/dotnet/*`) — replace with `cache-from: type=gha` on the subsequent buildx step, or mirror the bases into `ghcr.io/optivem/...` with a weekly refresh job. The retry is treating the symptom; the dep is mirrorable.
  - `Pre-warm Gradle Wrapper (retry transient CDN flakes)` — replace with `actions/cache` keyed on the wrapper version, or rely on `gradle/actions/setup-gradle@v6`'s built-in wrapper caching, which already handles this. The retry duplicates a problem the cache solves.
  - `retry_run gh ...` for GitHub API — **keep**; tier-2 mirror is genuinely impossible (you cannot mirror the GitHub API), and the retry hard-fails on 4xx. This is the legitimate tier-4 case.

  **Source:** Humble & Farley, *Continuous Delivery* ch. 4–5 (deterministic pipeline + commit-stage budget); Farley, *Modern Software Engineering* ch. 5 (managing complexity, eliminating variance); Farley's "Stop using flaky tests" / "Don't retry flaky tests" YouTube series; Mainstream-first principle at top of file.
...
- **Supply chain.** Flag actions whose `uses:` references point at a mutable ref (`@main`, `@master`, `@latest`, any branch name), or at an action from an untrusted author. Cite GitHub's hardening guide for GitHub Actions and SLSA level-3 build integrity.
  ...
  **Template / teaching-starter repo exception.** When the repository is intended for forking by other developers (academy course templates, project starters, reference architectures — repos whose primary value is being copied and adapted), major-tag pinning is preferred for **all** dependencies, including tier-2 single-maintainer ones. Forker-readability outweighs the marginal supply-chain reduction in a teaching context: SHA pins in a template force every forker to either understand and update them manually or carry stale pins forever, and `@<long-sha> # v2.3.1` syntax is illegible to readers learning the pattern. Major-tag pinning communicates version semantically and lets forks pull bug fixes automatically. Accept the residual supply-chain risk and document each tier-2 dependency in the template's README dependencies section so forkers can audit. **Applies to:** workflows in `shop` (academy course template), and any future repo whose `README.md` frames it as a template/starter/example. **Concrete example:** `Wandalen/wretry.action@v3` is acceptable in `shop/.github/workflows/*.yml` under this exception even though Wandalen is a single-maintainer source that would be tier-2 SHA-pinned in a non-template repo. Cite this exception in findings rather than flagging the major-tag pin as a tier-2 violation.
...
- **Bash strict mode — `set -euo pipefail` at top of every `run:` block and every standalone `.sh`.** Without it, bash's defaults are dangerous in CI: a failed command keeps the script running (`-e` fixes), an unset variable expands to empty and silently misbehaves (`-u` fixes), and a failed stage in a pipeline is masked by a successful later stage (`pipefail` fixes). All three flags should be set together as the first executable line. Flag any `shell: bash` `run:` block in `action.yml` / workflow YAML, or any standalone executable `.sh` file, that does not begin its script body with `set -euo pipefail`. Cite the failure mode the missing flag would mask (e.g. "the `retry_run gh api ... | jq .sha` pipeline can succeed with empty output if `retry_run` fails and `pipefail` isn't set").

  **Exception — sourced library scripts.** Files designed to be `source`d by callers (function libraries like `shared/gh-retry.sh`, `shared/ghcr-probe.sh`, `shared/remote-url.sh` — identifiable by a "Source this file from..." comment near the top, no top-level `main` logic) must NOT set `-euo pipefail` at file scope. Doing so contaminates the caller's shell options for the rest of its execution, which can break unrelated logic. Sourced libraries should rely on the caller having `set -euo pipefail` already, and use explicit per-function error handling where they need different semantics. Do not flag sourced libraries for lacking strict mode at file scope.
...
Applies to (non-exhaustive): `compose-prerelease-version`, `compose-release-version`, `read-base-version`, `bump-patch-versions`, `resolve-latest-prerelease-tag`, `resolve-latest-tag-from-sha`, `create-component-tags`, and any future action that treats "a version" as a first-class value.
...
**Divergence from SHA-pinning.** Industry security guides (OpenSSF Scorecard, GitHub workflow-hardening docs, supply-chain audit tooling) recommend pinning every third-party action to a 40-char commit SHA so a hijacked or maliciously retagged release cannot silently change CI behavior. This rubric deliberately diverges and uses major-tag pinning instead. The trade-off is intentional: this workspace is teaching material with a small, well-known set of marketplace actions, no PR-from-fork attack surface (`pull_request_target` is not used), and the secrets that do flow through CI are publishing/registry tokens (GHCR, Docker Hub, cross-repo PATs) rather than customer-data credentials — meaningful blast radius if compromised, but rotatable and scoped per-workflow rather than catastrophic. Major tags keep lessons readable, keep upgrade cadence simple (bump on major, pick up patches automatically), and avoid the Dependabot/PR churn a SHA-pin policy generates. Re-evaluate if the risk profile changes — external contributors landing code that runs in CI, customer-data credentials entering workflows, or `pull_request_target` being adopted — at which point a SHA-pinning pass is the right answer.
...
- **v11 (2026-04-27)** — added a **Template / teaching-starter repo exception** to the §1.4 supply-chain pin-strategy ladder. For repos intended for forking by other developers (academy course templates, project starters, reference architectures — repos whose primary value is being copied and adapted), major-tag pinning is preferred for all dependencies, including tier-2 single-maintainer ones. Forker-readability outweighs the marginal supply-chain reduction in a teaching context. Concrete example codified: `Wandalen/wretry.action@v3` is acceptable in `shop/.github/workflows/*.yml` even though Wandalen is single-maintainer.
- **v10 (2026-04-27)** — extended §1.3 with two retry-related rules: **"Don't add retry without measured failure data"** (require ≥3 failures in ~30 days OR a cited incident before wrapping a step in retry; mainstream actions like `docker/login-action` and `actions/checkout` deliberately leave job-level rerun as the recovery layer) and **"Determinism over retry — mirror external deps rather than wrap them in retry"** (Farley CD framing: 5-tier response hierarchy from "fix root cause" → "mirror the dep" → "rerun the job" → "retry the operation" → "tolerate the flake rate"; flag tier-4 retries where tier-2 caching/mirroring is feasible — concrete examples: `Pre-pull base images` retry → `cache-from: type=gha` or GHCR mirror; `Pre-warm Gradle Wrapper` retry → `actions/cache` or `setup-gradle@v6` built-in cache; `gh_retry` is the legitimate tier-4 case because the GitHub API is unmirrorable). Tightened the existing bounded-retry rule with the transient-only error pattern and pointers to the `gh_retry` and `check-ghcr-image-tag-exists` reference implementations.
