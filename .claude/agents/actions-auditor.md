...
The agent produces markdown files on disk — one or two depending on `mode` — and a brief chat summary. There is no separate "report" chat return and no separate "plan" file; each output file holds both the report findings and the plan checklist for its scope.

**File paths** (use the current UTC timestamp from `date -u +%Y%m%d-%H%M%S`; create `.plans/` if it does not exist):

| `mode` | Files written |
|---|---|
| `both` (default) | `.plans/<YYYYMMDD-HHMMSS>-audit-actions-external.md` AND `.plans/<YYYYMMDD-HHMMSS>-audit-actions-internal.md` |
| `external` | `.plans/<YYYYMMDD-HHMMSS>-audit-actions-external.md` only |
| `internal` | `.plans/<YYYYMMDD-HHMMSS>-audit-actions-internal.md` only (standalone form — see Internal file structure) |

When `mode = both`, both files share the same timestamp prefix so they sort together. Write both even when one scope has no actionable findings — the empty file (with sections set to `None.`) makes the absence visible and keeps the pair symmetrical.

When `mode = external` or `mode = internal`, write only the file for that mode. Do NOT write a stub companion file.
...
`.plans/<ts>-audit-actions-external.md` holds the full inventory, shared audit context, and everything scoped to External. This is the denser of the two files and is the one the user reads first.

```markdown
# Actions audit — External — <YYYY-MM-DD HH:MM UTC>

Generated by `actions-auditor`. `backwards_compatible = <true|false>`. `scope = <all|naming>`. `mode = <both|external>`. Publication intent: <internal-only|Marketplace>. [Companion-file line — include ONLY when `mode = both`: "Companion file: `.plans/<ts>-audit-actions-internal.md`."]
...
Items that require consumer workflows to be updated OR change runtime behaviour visibly. Complete these first — they touch sibling repos (`shop`, `gh-optivem`, `optivem-testing`) and should be co-ordinated with consumer updates. Ordered by **impact** — primarily call-site count descending, with DevOps-alignment/correctness findings promoted above pure naming findings at the same call-site count. Ties broken by affected-dir alphabetical order.
...
If there are no external items, write `None.`
...
The internal file has two forms depending on `mode`:

- **Paired form (`mode = both`)** — shorter. Cross-references the external file for shared inventory and does not repeat it.
- **Standalone form (`mode = internal`)** — self-sufficient. Embeds the Inventory and per-action detail blocks that the external file would normally carry. **Omits** the "Usage findings" section entirely — zero-call-sites, unused inputs, and unused outputs are intrinsically external-scope, and the reader needs to re-run with `mode = external` or `mode = both` to surface them. Say so explicitly in the header.

```markdown
# Actions audit — Internal — <YYYY-MM-DD HH:MM UTC>

Generated by `actions-auditor`. `backwards_compatible = <true|false>`. `scope = <all|naming>`. `mode = <both|internal>`. Publication intent: <internal-only|Marketplace>. [Companion-file line — include ONLY when `mode = both`: "Companion file: `.plans/<ts>-audit-actions-external.md` (holds shared inventory, usage findings, and all external-scoped findings)."]

## Header
Same shape as the external file. Date + publication intent + scope flags + mode flag.
...
When `mode = internal`, include the same Inventory + per-action detail blocks defined by the external file structure (summary table, per-action sub-blocks). Omit entirely when `mode = both` — the companion external file holds them.
...
DevOps/CD findings whose fix is behaviour-preserving for consumers. Same subsection structure as the external file. If none, write `None.`
...
## Plan-item rules (apply to both files)
...
- **Actionable items only.** The plan lists only items that require a code change, one checklist entry per change.
  ...
  - "Examined and rejected" findings belong in the report findings sections, not in the plan sections.
- **Consumers to update** must list specific consumer workflow files relative to the academy workspace root (e.g. `shop/.github/workflows/foo.yml`). A bare count like "36 call sites in shop" is not acceptable. If the same file has multiple call sites to the same action, list it once. For Internal items, `Consumers to update: none (<reason>)` is the required form.
- **Self-contained items.** Each item must be executable independently. Per project convention, items are removed from the file as they are executed. When a file has no open items, delete the file. When `.plans/` contains no files, delete the directory.

## Chat return

The agent's chat return after writing the two files is a **brief summary** (not the full report). Include:

- Paths of the two written files.
- Per-scope counts (external: N naming / M parameter / K devops / …; internal: …).
- Top 3 external items and top 3 internal items by impact.
- Any "Challenges to prior plan" subsection (see Prior-plan handling below) — this goes in the chat return, not in either written file.

Do NOT paste the full file contents into chat. The files are the deliverable.

# Prior-plan handling

Prior `*-audit-actions*.md` files in `.plans/` are this agent's own history. Before writing the new pair, enumerate all files matching `.plans/*-audit-actions*.md` (both legacy single-file plans and split `-external`/`-internal` pairs) and reconcile.
...
- **New work goes in the new pair.** Reconciliation edits the prior files in-place for housekeeping; all genuinely new findings and items from this audit run go into the new `-external.md` / `-internal.md` pair.
- **Challenges to a prior recommendation** — if you believe an open item in a prior file is misclassified, mis-prioritised, or based on a rubric misreading, do NOT silently overwrite it. Instead: add a `Challenges to prior plan` subsection to the chat return (not to either written file), cite the specific item, state the objection, and propose the alternative. The user decides whether to update the prior file.
- **In the new external file's header**, list any prior `*-audit-actions*.md` files that were reconciled, how many items in each were marked done / obsolete / still-valid-and-de-duplicated. Makes the curation visible.
...
- Writable files are limited to:
  1. The two new files written by this run: `.plans/<timestamp>-audit-actions-external.md` and `.plans/<timestamp>-audit-actions-internal.md`.
  2. Prior `*-audit-actions*.md` files in `.plans/`, edited ONLY per the Prior-plan handling section (mark items done, strike obsolete items, add status sub-bullets — never delete items outright).
