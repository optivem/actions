...
The agent produces **two self-contained markdown files on disk** — one per consumer-visibility scope — and a brief chat summary. There is no separate "report" chat return and no separate "plan" file; each output file holds both the report findings and the plan checklist for its scope.

**File paths** (use the current UTC timestamp from `date -u +%Y%m%d-%H%M%S`; create `.plans/` if it does not exist):

```
.plans/<YYYYMMDD-HHMMSS>-audit-actions-external.md
.plans/<YYYYMMDD-HHMMSS>-audit-actions-internal.md
```

Both files share the same timestamp prefix so they sort together. Write both even when one scope has no actionable findings — the empty file (with sections set to `None.`) makes the absence visible and keeps the pair symmetrical.
...
Generated by `actions-auditor`. `backwards_compatible = <true|false>`. `scope = <all|naming>`. Publication intent: <internal-only|Marketplace>. Companion file: `.plans/<ts>-audit-actions-internal.md`.
...
`.plans/<ts>-audit-actions-internal.md` is shorter. It cross-references the external file for shared inventory and does not repeat it.

```markdown
# Actions audit — Internal — <YYYY-MM-DD HH:MM UTC>

Generated by `actions-auditor`. `backwards_compatible = <true|false>`. `scope = <all|naming>`. Publication intent: <internal-only|Marketplace>. Companion file: `.plans/<ts>-audit-actions-external.md` (holds shared inventory, usage findings, and all external-scoped findings).

## Header
Same shape as the external file. Date + publication intent + scope flags.
