...
7. **DevOps alignment pass.** Walk every action against the dimensions in rubric §1 (build-once-promote-many, idempotence, fail-fast, fast-feedback sizing, rate-limit awareness, bounded retry with backoff, secrets, supply chain, observability dual surface, shell portability, `branding:`), the architectural principles in §5–§7 (primitives vs. composites, one-concern-per-action, composition order, idempotence), and the filing guide in §8. Respect the forward-looking exemptions in §2.
   ...
   **§1.9 marketplace-action version currency sub-pass.** For every `uses:` ref encountered in the `runs.steps:` block of an audited action (and in its consumers' workflow files during the usage scan), apply rubric §1.9 explicitly:

   1. Collect every `uses: <owner>/<repo>[/path]@<ref>` inside scoped `action.yml` files (`runs.steps.*.uses:`). Drop local refs (`./`, `../`) and first-party internal refs (`optivem/actions/*`, `optivem/<name>-action` — those are governed by the internal release cadence, not upstream Marketplace).
   2. Deduplicate by `<owner>/<repo>` and query `gh api repos/<owner>/<repo>/releases/latest --jq .tag_name` once per distinct repo (cache for the audit run). Cap at ~60 distinct repos; if exceeded, say so in the report and proceed with partial results.
   3. For each ref, classify under the §1.9 categories — **D** (behind latest major), **E** (mixed majors across audited actions), **F** (deprecated / archived upstream — migrate, don't bump). Flag anti-patterns (`@master` / `@main` floats, undocumented patch pins) alongside.
   4. File findings under the DevOps alignment section's **Other** subsection, tagged `§1.9`. External-scope; classify breaking vs non-breaking per the §1.9 filing guide (default to breaking when unsure).
   5. Do NOT flag first-party internal actions under this sub-pass even when they appear behind upstream peers — they are out of scope per rubric §1.9.
...
- [ ] **1. <short title>** — <one-line description of the change>
  - Affects: `<dir1>`, `<dir2>`
...
Items that change the action's public surface but leave existing consumers working unchanged (safe cap defaults, additive outputs, relaxed `required`, deprecation messages that name a replacement, new optional inputs with behaviour-preserving defaults). Safe to land independently of consumer updates — consumers adopt at their own pace. Ordered by impact within this section using the same rules as the breaking list. Numbering continues from the Breaking section (so if Breaking has items 1–3, the first Non-breaking item is 4).

- [ ] **<N>. <short title>** — <one-line description of the change>
  - Affects: `<dir1>`, `<dir2>`
...
- **Numbered items.** Every item in a plan file starts with a sequential integer followed by a period inside the bold title — e.g. `- [ ] **1. <short title>** — <description>`. Numbering is **continuous across all sections of a single plan file** (e.g. Breaking items 1–3 are followed by Non-breaking items 4–5 in the external plan; Internal items start at 1 in the internal plan, which is independent). This makes items easy to reference in conversation ("execute item 3"). Numbers are **stable identifiers** — when an item is deleted during plan execution, do NOT renumber the remainder; leave gaps so `item 3` keeps referring to the same work from creation through completion.
