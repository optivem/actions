...
The plan file contains **only actionable items that require a code change**, one checklist entry per change.
...
**Ordering — External before Internal.** The plan is split into two top-level sections, in this order:

1. **## External Changes** — items that force any consumer workflow to be updated, or that change runtime behaviour visibly from a consumer's point of view. Examples: action directory renames, input or output renames, input or output removals, output polarity flips, splitting one action into two, adding a new required input, adding a new default that caps previously-unbounded behaviour (e.g. a new `timeout-seconds` default that is lower than today's effective runtime), changing `required:` from `false` to `true`, removing a previously-valid input value.
2. **## Internal Changes** — items with no consumer-visible surface change. Examples: `description:` edits, adding an optional input whose default preserves today's behaviour exactly, metadata-only edits (`author:`, `branding:`), internal script refactors, internal idempotence/retry tweaks that do not change the success/failure contract, fixing a `required-implicit` field by making the already-effective `required: false` explicit.

Within each of the two sections, order items by **impact** — primarily call-site count descending (more call sites = higher priority), with DevOps-alignment findings that affect correctness or safety promoted above pure naming findings at the same call-site count. Ties broken by affected-dir alphabetical order for determinism.

If a section has no items, still include the heading followed by a single line `None.` so the split is always visible.

**Tie-breaker for ambiguous items.** If you are unsure whether an item is External or Internal — e.g. adding an input with a new default that MIGHT change behaviour for some callers — classify as **External** and note the condition in the item body. It is safer to over-report breaking changes than to bury one in the Internal section.
...
**Consumers to update** must list the specific consumer workflow files (relative to the academy workspace root, e.g. `shop/.github/workflows/foo.yml`) that reference the affected action(s). A bare count like "36 call sites in shop" is not acceptable — the user needs to see exactly which files are touched. If the same file has multiple call sites to the same action, list it once. For Internal items, `Consumers to update: none (<reason>)` is the required form.
